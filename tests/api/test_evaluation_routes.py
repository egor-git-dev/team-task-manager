...
def test_create_evaluation_success(client, override_current_user, monkeypatch):
...
def test_create_evaluation_task_not_found_error(
...
def test_create_evaluation_permission_error(client, override_current_user, monkeypatch):
    override_current_user(User(id=2, team_id=1, role=UserRole.MANAGER))
...
def test_create_evaluation_task_not_done_error(
    client, override_current_user, monkeypatch
):
    override_current_user(User(id=2, team_id=1, role=UserRole.MANAGER))
    mock_create_evaluation = AsyncMock(
...
def test_create_evaluation_task_has_no_assignee_error(
    client, override_current_user, monkeypatch
):
    override_current_user(User(id=2, team_id=1, role=UserRole.MANAGER))
    mock_create_evaluation = AsyncMock(
...
def test_create_evaluation_already_exists_error(
    client, override_current_user, monkeypatch
):
    override_current_user(User(id=2, team_id=1, role=UserRole.MANAGER))
    mock_create_evaluation = AsyncMock(
...
def test_get_my_evaluations_success(client, override_current_user, monkeypatch):
    evaluations = [
        Evaluation(
            id=1,
            task_id=5,
    ...
    override_current_user(User(id=3, team_id=1, role=UserRole.USER))
...
def test_get_my_average_score_success(client, override_current_user, monkeypatch):
    override_current_user(User(id=3, team_id=1, role=UserRole.USER))
